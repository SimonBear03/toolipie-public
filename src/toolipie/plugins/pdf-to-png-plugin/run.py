...
def _render_one_page(
    ...
    try:
        try:
            ...
            return base_name, page_number, False, f"pypdfium2 import failed: {ie}"
...
def run(
    ...
    # Lazy import pypdfium2 here as well (module has native code)
    try:
        import pypdfium2 as pdfium  # type: ignore
    except Exception as e:
        raise RuntimeError(f"pypdfium2 not available: {e}")
    ...
    with Progress(
        ...
        # Determine parallelism: default conservatively on macOS for PDFium stability
        if ctx.workers and ctx.workers > 0:
            jobs = int(ctx.workers)
        else:
            try:
                import platform
                env_threads = int(os.environ.get("TOOLIPIE_PDF_THREADS", "0"))
            except Exception:
                env_threads = 0
            if env_threads > 0:
                jobs = env_threads
